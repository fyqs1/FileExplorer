...
enum FileSortMode: String, CaseIterable, Identifiable {
    ...
    var title: String {
        ...
        case .dateNewest: return "修改时间 新→旧"
        case .dateOldest: return "修改时间 旧→新"
        case .sizeLargest: return "大小 大→小"
        case .sizeSmallest: return "大小 小→大"
        }
    }
    ...
    var systemImage: String {
        ...
        case .sizeLargest, .sizeSmallest: return "arrow.up.arrow.down"
        }
    }

    static let storageKey = "fileexplorer.sortMode"
...
enum FileTreeLoader {
    ...
    static let ioQueue = DispatchQueue(label: "com.fyqs.FileExplorer.fileio", qos: .userInitiated)
    ...
    /// Resolve directory path for browsing (critical for rootless `/var/jb`).
    ...
    static func listChildren(at path: String, includeHidden: Bool = true) -> FileListOutcome {
        ...
        do {
            ...
            FileSortMode.sort(&nodes, mode: .nameAsc)
            return .listed(nodes)
        } catch {
            do {
                ...
                return .unreadable(message.isEmpty ? "无法读取此目录" : message)
            }
        }
    }
    ...
    static func copyForSharing(path: String) -> Result<URL, Error> {
        ...
        guard fm.fileExists(atPath: path) else {
            return .failure(fileError(1, "文件不存在"))
        }
        let src = URL(fileURLWithPath: path)
        let folder = fm.temporaryDirectory.appendingPathComponent(
            "fileexplorer-share-\(UUID().uuidString)",
            isDirectory: true
        )
        do {
    ...
    static func removeShareStaging(around fileURL: URL) {
        ...
        guard folder.lastPathComponent.hasPrefix("fileexplorer-share-") else { return }
    ...
    static func loadTextPreview(path: String) -> Result<String, Error> {
        ...
        if size > maxPreviewBytes {
            return .failure(fileError(2, "文件过大，无法预览"))
        }
        do {
            ...
            if nonPrintable > sample.count / 8 {
                return .failure(fileError(3, "不是文本文件"))
            }
    ...
    static func createDirectory(named name: String, in parent: String) -> Result<String, Error> {
        ...
        guard !trimmed.isEmpty, !trimmed.contains("/") else {
            return .failure(fileError(10, "无效的文件名"))
        }
